...
BackgroundImage::~BackgroundImage() { delete (stars); }

void BackgroundImage::draw(Renderer& renderer) { stars->invoke_renderer(renderer); }
...
void Terrain::draw(Renderer& renderer) { hills->invoke_renderer(renderer); }
...
void Lander::update(float frame_time)
{
    ...
    if (engine_flame_percent > 0) {
        physics->apply_force(rotate_point(SDL_FPoint{0, float(-0.5 * engine_flame_percent)},
